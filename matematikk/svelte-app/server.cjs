...
app.get('/api/getQuestions', (req, res) => {
    const sql = "SELECT * FROM mathquestion LIMIT 5";
    db.all(sql, [], (err, rows) => {
        if (err) {
            throw err;
        }
...
app.get('/api/getAlternatives/:id', (req, res) => {
    const sql = "SELECT * FROM alternatives WHERE question_id = ?";
...
app.get('/api/getLatexHint/:id', (req, res) => {
    const sql = "SELECT * FROM latex_hint WHERE question_id = ?";
    db.all(sql, [req.params.id], (err, rows) => {
        if (err) {
            throw err;
        }
